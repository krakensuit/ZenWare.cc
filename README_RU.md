...
| `ZenWare.Loader` | GUI-лоадер, только локальная сборка | LoadLibrary инжект (код manual-map есть, но не подключён), темы, 8 языков | локальный `dist/ZenWare.exe`, не публикуется |
...
(`VEngineCvar007` ищется сначала в `vstdlib.dll`) → загрузка конфига →
диагностика нетваров → менеджер отрисовки → хуки `MinHook`. Отсутствующий
паттерн/интерфейс = окно с ошибкой, а не краш.

Каждый кадр работают две цепочки:

- **Тик** (`ClientMode::CreateMove`) — движение (BunnyHop → AutoStrafe →
  JumpStats), затем бой (Aimbot → TriggerBot → AutoPistol → NoSpread), затем
  AutoShove. Оригинал движка вызывается ровно раз за тик, результат
  переиспользуется.
...
один файл. Инжект — удалённый LoadLibraryW (реализация manual-map с
релокациями/импортами/затиранием заголовков есть в коде, но сейчас не
подключена к кнопке INJECT). Сети нет, автообновления нет.
...
- **Помощники** — TriggerBot (стреляет по лучу после аима, без лага в тик),
  ...
  14 стволов, включён по умолчанию)
...
| `MOUSE4` (держать) | Распрыжка (альт. клавиша) |
...
- Слоты рядом с путём DLL игры: `ZenWare.cfg`, `ZenWare2.cfg`,
  `ZenWare3.cfg` — plain `key=value`, 121 ключ. Сессионные значения
